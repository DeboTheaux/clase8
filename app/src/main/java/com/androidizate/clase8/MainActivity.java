...
public class MainActivity extends AppCompatActivity implements Callback<List<User>> {
    ...
    private void downloadInfo() {
        Book book = new Book();
        book.setID(1);
        book.setDescription("DESCRICION NUEVA");
       //restApiClient.getAllUsers().enqueue(this);
        restApiClient.putBook(1, book).enqueue((new Callback<Book>() {
            @Override
            public void onResponse(Call<Book> call, Response<Book> response) {
                if (response.isSuccessful()) {
                    Toast.makeText(MainActivity.this, "Bien" + response.body(), Toast.LENGTH_LONG).show();
                }
            }
